...
const chassis_thick = 2;
const chassis_armature_housing_thick = 1.5;
const chassis_armature_housing_height = 5;
...
const base_lead_len = 1.5;
const base_lead_pad = 2;
...
const plotter_chassis_bottom_rail_height = 8;
const plotter_chassis_rail_vert_spacing = 8;
...
function pen_chassis() {
  const armatureHousingCenterX = base_width/2 + chassis_armature_housing_thick;
  const armatureHousingCenterY = base_len/2 + chassis_armature_housing_thick;  
  ...
  const base_plate = subtract(
    ...
    cuboid({ size: [base_lead_len, armature_housing_len, chassis_thick ],
             center: [(chassis_width-base_lead_len)/2-armature_housing_width, (chassis_len-armature_housing_len)/2, 0] }),
  ...
  const armature_housing = subtract(
    ...
    // Etches for base leads
    cuboid({ size: [ base_width+2*base_lead_width, base_lead_len+spacing_epsilon, chassis_armature_housing_height ],
             center: [ armatureHousingCenterX, chassis_armature_housing_thick+base_lead_pad+base_lead_len/2, 0 ]}),

    cuboid({ size: [ base_width+2*base_lead_width, base_lead_len+spacing_epsilon, chassis_armature_housing_height ],
             center: [ armatureHousingCenterX, chassis_armature_housing_thick+base_len-base_lead_pad-base_lead_len/2, 0 ]}),
...
function main() {
  ...
  /*return union(
    armatureAndHolder(), 
    translate(
      [-chassis_width/2+(base_width+chassis_armature_housing_thick), 
        -1*chassis_armature_housing_thick, -0.9], 
      chassis()
    )
  );*/
}
